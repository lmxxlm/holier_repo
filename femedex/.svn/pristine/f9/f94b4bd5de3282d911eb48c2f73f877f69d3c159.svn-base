	...
	<div class="container-fluid">
		...
		<div id="content">
			...
			<ul id="myTab" class="nav nav-tabs">
                ...
                <li class="positionRelative active"><a href="javascript:void(0)" onFocus="this.blur()">提现驳回<c:if test="${acount != 0}">（${acount}）</c:if></a></li>
            </ul>       
				...
				<div class="addPat col-md-3 col-xs-3">
				    <div class="input-group">
						<input type="text" class="form-control" name="search_date"
							placeholder="请选择预订时间" value="${search_date}">
                        <span class="input-group-addon"><i class="glyphicon glyphicon-search" style="color:black;"></i> </span>
					</div>
				</div>
				-->
			    ...
			    <div align="center">
					 <ul class="pagination">
						 <li <c:if test="${searchParam.cp == 1}"> class="disabled"</c:if>><a
							 href="${mainUril}withdrawReject.html?cp=${uro.pageView.back}&&ps=${searchParam.ps}">上一页</a>
						 </li>
						 <li     
							 <c:if test="${uro.pageView.last == searchParam.cp}"> class="disabled"</c:if>><a
							 href="${mainUril}withdrawReject.html?cp=${uro.pageView.next}&&ps=${searchParam.ps}">下一页</a>
						 </li>
					 </ul>
