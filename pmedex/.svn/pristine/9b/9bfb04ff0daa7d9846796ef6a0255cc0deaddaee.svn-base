			...
			<div class="mainBody">
				...
				<table class="table table-striped table-hover">
					...
					<tbody>
						...
						<c:forEach items="${service_list}" var="record">
							<tr>
								<td>${record.service_content}</td>
								<td>${record.validity_time}</td>
								<td>${record.service_price}</td>
								<td>${record.is_valid}</td>
								<td>${record.remark}</td>
								<td class="td1"><a
									href="javascript:void(0)" data-id="${record.service_id}"
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group">
						...
						-->
						<input class="form-control" name="validity_time" placeholder="请输入有效期" style="width:30%;" readonly value="">
					</div>
					...
					<div class="form-group">
						...
						<div>
							<label class="radio-inline"><input type="radio"
								name="is_valid" value="true" checked>启用</label> <label
								class="radio-inline"><input type="radio" name="is_valid"
								value="false">不启用</label>
						</div>
					</div>
					<!-- 
					<div class="form-group ">
	...
	$(function(){
    	...
    	$('.editService').click(function(){
    		...
    		$.post(questURL,questArr,function(result){
    			if(result.status){
    		    	...
    		    	$('#adminModal textarea[name="backup"]').val(result.data.remark);
    	...
    	$(".deleteService").click(function(){
    	    ...
    	    if(confirm('确定要把产品“'+name+'"从列表中删除吗？')){
				...
				var questArr = {service_id:id,token:token};
