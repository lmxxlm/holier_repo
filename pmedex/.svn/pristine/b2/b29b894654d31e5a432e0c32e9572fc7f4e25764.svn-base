...
<body>
		...
		<div id="content">
			...
			<ul id="myTab" class="nav nav-tabs">
			   <li ><a href="withdrawForRecureOrder.html" class="urlConvert" data-val="0" onFocus="this.blur()">待提现</a></li>
                ...
                <li class="active"><a href="rejectForTransfers.html" class="urlConvert" data-val="2" onFocus="this.blur()">已驳回</a></li>
            </ul>       
            <form method="post" id="searchForm" action="${mainUrl}search_transfers_withdraw.do">
            <input type="hidden" name="search_status" value="3">
					    ...
					    <c:forEach  items="${data}" var="record">
					        <tr>
							   <td><a href="javascript:void(0)" data-id="${record.orderid }" class="orderDetail">${record.orderid }</a></td>
								...
								<td><a href="javascript:void(0)" data-id="${record.recordid }"  data-payAccountId="${record.pay_accountid}" data-orderid="${record.orderid}" data-payAccountName="${record.pay_account_name}" data-amount="${record.amount}"  class="rejectDetail">查看原因</a></td>
							</tr>
        ...
        <div class="modal fade" id="rejectReasonModal" data-backdrop="false" tabindex="-1" role="dialog" aria-labelledby="rejectModalLabel" aria-hidden="true">
           <div class="modal-dialog">
              <div class="modal-content">
                 ...
                 <div class="modal-body">
                    <div style="padding-bottom:15px;">订单号：<span class="orderId"></span>，账号：<span class="payAccountId"></span>，用户名：<span class="payAccountName"></span>，取现金额：<span class="amount"></span></div>
	...
	$(function(){
		...
		$('a.rejectDetail').click(function(){
	        ...
	        $('#rejectReasonModal .orderId').html(orderid);
	        $('#rejectReasonModal .payAccountId').html(payAccountId);
	        $('#rejectReasonModal .payAccountName').html(payAccountName);
	        $('#rejectReasonModal .amount').html(amount);
	        ...
	        var questURL=runParams.mainUrl+'get_reject_kf_transfer_bill_reason.do';
	        
	        $.post(questURL,questArr,function(result){
	        	if(1==result.status){
	        		$('#rejectReasonModal textarea[name="rejectReason"]').val(result.data);
