	...
	<div class="container-fluid">
		...
		<div id="content">
			...
			<ul id="productStatus" class="nav nav-tabs">
			   <li><a href="javascript:void(0)" data-link="order_qrdd.html" class="urlConvert"  data-toggle="tab" onfocus="this.blur()">确认订单</a></li>
			   <li class="active"><a href="javascript:void(0)" data-link="order_cxqrdd.html" class="urlConvert" data-toggle="tab" onfocus="this.blur()">重新确认订单</a></li>
		    </ul>
			<form method="post" id="searchForm" action="${mainUrl}search_cxqrdd.do">
			...
			<div class="mainBody">
					...
					<tbody>
						...
						<c:forEach items="${oro.records}" var="order">
							...
							<tr>
								...
								<td>${order.confirmedCheck}</td>
								<td class="td1">
								<a href="orderDetail.html?postType=cxqrdd&id=${order.orderId}">
								<span class="glyphicon glyphicon-zoom-in"></span>重新确认订单</a>&nbsp;&nbsp;
								<a href="javascript:void(0)" data-id="${order.orderId}" class="cancellation">
								<span class="glyphicon glyphicon-remove"></span>订单作废</a>
								</td>
							</tr>
							</c:if>
						</c:forEach>
				...
				<div align="center">
					<ul class="pagination">
						...
						<!-- <a href="${mainUril}order_cxqrdd.html?cp=${oro.pageView.back}&&ps=${searchParam.ps}">上一页</a> -->
							<a href="javascript:void(0);" class="pre">上一页</a>
						</li>
						<li     
							<c:if test="${oro.pageView.last == searchParam.cp}"> class="disabled"</c:if>>
						<!-- <a href="${mainUril}order_cxqrdd.html?cp=${oro.pageView.next}&&ps=${searchParam.ps}">下一页</a> -->
           ...
           <div class="modal-dialog">
              <div class="modal-content">
                 <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="adminModalLabel">驳回订单--<span id="orderId">${order.orderId}</span></h4>
                 </div>
                 <div class="modal-body">
                    <div class="form-group">
                        <!--  <label for="reason">添补理由</label>-->
                        <textarea class="form-control notebook" rows="4" name="rejectReason" placeholder="驳回原因"></textarea>
                    </div>
                 </div>
