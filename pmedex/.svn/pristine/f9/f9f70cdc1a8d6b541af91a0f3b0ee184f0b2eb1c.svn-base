$(function(){
	...
	$('.detail').click(function(){
		...
		//根据id获取这条数据
		var questURL = runParams.mainUrl+'/getJbOperationById.do';
		var questArr = {
			id:id
		};
	...
	function doAdd(){
		...
		var doctorId = $('#adminModal input[name="accountId"]').val();
		...
		var questURL = runParams.mainUrl+'addJBOperation.do';
		...
		$.post(questURL,questArr,function(result){
			if(1==result){
				location.href=runParams.mainUrl+'JBOperation.html';
			}else{
				alert('添加失败！');
				return false;
			}
