	...
	<div class="container-fluid">
		...
		<div id="content">
			...
			<ul id="productStatus" class="nav nav-tabs">
			   <li class="active"><a href="javascript:void(0)" data-link="order_qrdd.html" class="urlConvert"  data-toggle="tab" onfocus="this.blur()">确认订单</a></li>
			   <li><a href="javascript:void(0)" data-link="order_cxqrdd.html" class="urlConvert" data-toggle="tab" onfocus="this.blur()">重新确认订单</a></li>
		    </ul>
			<form method="post" id="searchForm" action="${mainUrl}search_qrdd.do">
			...
			<div class="mainBody">
					...
					<tbody>
						...
						<c:forEach items="${oro.records}" var="order">
							...
							<tr>
								...
								<td class="td1">
								<a href="orderDetail.html?postType=qrdd&id=${order.orderId}">
								<span class="glyphicon glyphicon-zoom-in"></span>确认订单</a>&nbsp;&nbsp;
				...
				<div align="center">
					<ul class="pagination">
						...
						<!-- <a href="${mainUril}order_qrdd.html?cp=${oro.pageView.back}&&ps=${searchParam.ps}">上一页</a> -->
						<a href="javascript:void(0);" class="pre">上一页</a>
						</li>
						<li     
							<c:if test="${oro.pageView.last == searchParam.cp}"> class="disabled"</c:if>>
						<!-- <a href="${mainUril}order_qrdd.html?cp=${oro.pageView.next}&&ps=${searchParam.ps}">下一页</a> -->
