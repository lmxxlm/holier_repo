...
<!--[if gt IE 8]><!--> <html class="no-js" lang=”zh”> <!--<![endif]-->
    ...
    <body>
                        ...
                        <div class="calendarList">
                            ...
                            <table border="1" cellspacing="0" cellpadding="0" class="infoTable" width="100%">
                              ...
                              <tr>
                                ...
                                <td class="td2" class="ptpAmount">${ptp.amount}</td>
                              ...
                              <tr>
                                ...
                                <td class="td2" class="ptlAmount">${ptl.amount}</td>
                                <td class="td1">收款日期：</td>
                              ...
                              <tr>
                                ...
                                <td class="td3" colspan="6">
                                    ...
                                    <c:if test="${order.status == '重新确认'}"><button type="button" class="btn btn-primary" onclick="location.href='orderConfirmAgain.html?orderId=${order.orderId}'"><span class="glyphicon glyphicon-edit"></span>&nbsp;&nbsp;订单确认</button></c:if>
                                    <c:if test="${order.status == '已支付'}"><button type="button" class="btn btn-primary" onclick="serviceComplete1()"><span class="glyphicon glyphicon-edit"></span>&nbsp;&nbsp;申请派发积分</button></c:if>
                                    <c:if test="${order.status == '未支付'}"><button type="button" class="btn btn-primary" onclick="showPayInfo('定金')"><span class="glyphicon glyphicon-edit"></span>&nbsp;&nbsp;确认定金</button></c:if>
                                    <c:if test="${order.status == '已确认'}"><button type="button" class="btn btn-primary" onclick="showPayInfo('余款')"><span class="glyphicon glyphicon-edit"></span>&nbsp;&nbsp;确认余额</button>&nbsp;&nbsp;<button type="button" class="btn btn-default" onclick="showCancelInfo()"><span class="glyphicon glyphicon-arrow-left"></span>&nbsp;&nbsp;重新确认</button></c:if>
                                    <c:if test="${order.status == '积分待派发'}"><button type="button" class="btn btn-primary" onclick="serviceComplete2()"><span class="glyphicon glyphicon-edit"></span>&nbsp;&nbsp;派发积分</button></c:if>   
                                    ...
                                    <div class="bookOrder payInfo">
										...
										<input name="payStatusType" type="hidden" value="">
								    ...
								    <div class="bookOrder cancelInfo">
								        <div class="form-group">
											<label for="cancelReason">驳回原因</label>
                                            <textarea class="form-control" name="cancelReason" placeholder="请输入驳回原因" ></textarea>
										</div>
								        <div class="form-group">
											<label for="OPName2">操作人</label>
                                            <input type="text" class="form-control" name="OPName2" value="${currentUserName}" placeholder="请输入操作人" autocomplete="off">
										    <button type="button" class="btn btn-primary btn-block cancelOrder" style="margin-top:20px;"><span class="glyphicon glyphicon-ok"></span>&nbsp;&nbsp;重新确认订单</button>
										</div>
								    </div>
                                </td>
                              </tr>
        ...
        <div class="modal fade" id="scoreModal" data-backdrop="false" tabindex="-1" role="dialog" aria-labelledby="scoreModalLabel" aria-hidden="true">
           <div class="modal-dialog">
              <div class="modal-content">
                 <div class="modal-header">
                    <h4 class="modal-title" id="scoreModalLabel">金币变化一览</h4>
                 </div>
                 <div class="modal-body">
                    <dl>
                      <dt>上级医生（${order.owner.name}）：</dt>
                      <dd style="margin-top:10px;">派发前：<span id="old_high_score"></span>&nbsp;&nbsp;&nbsp;&nbsp;派发后：<span id="high_score"></span>&nbsp;&nbsp;&nbsp;&nbsp;变化：<span id="high_score_change"></span></dd>
                    </dl>
                    <c:if test="${order.requester_type eq '医生'}">
                    <dl>
                      <dt>下级医生（${order.requesterName}）：</dt>
                      <dd style="margin-top:10px;">派发前：<span id="old_low_score"></span>&nbsp;&nbsp;&nbsp;&nbsp;派发后：<span id="low_score"></span>&nbsp;&nbsp;&nbsp;&nbsp;变化：<span id="low_score_change"></span></dd>
                    </dl>
                    </c:if>
                 </div>
                 <div class="modal-footer">
                    <button type="button" class="btn btn-primary flushData">确定</button>
                 </div>
              </div>
           </div>
        </div>
        ...
        function showPayInfo(type){
        	$('input[name="payStatusType"]').val(type);
        	if('定金'==type){
        		$('input[name="pay_amount"]').val($('.ptpAmount').html());
        	}
        	if('余款'==type){
        		$('input[name="pay_amount"]').val($('.ptlAmount').html());
        	}
    		$('.bookOrder').hide();
        ...
        function serviceComplete1(){
        	if(confirm('该服务所有流程已经完成了吗？')){
            	...
            	var questURL = runParams.mainUrl+'orderCheckComplete.do';
        		var questArr = {
        			orderId:orderId,
        			token:token
        		};
        		$.post(questURL,questArr,function(result){
        			if(1==result){
        				alert('操作成功！');
        				location.href=runParams.mainUrl+"order_sqpfjf.html";
        			}else{
        				alert('操作失败！');
        				return false;
        			}
        ...
        function serviceComplete2(){
        	if(confirm('该服务所有流程已经完成了吗？')){
            	...
            	var questURL = runParams.mainUrl+'get_user_score_balance.do';
        		var questArr = {
        			highDoc:"${order.owner.ownerId}",
        			lowDoc:"${order.requester}"
        		};
        		$.post(questURL,questArr,function(result){
        			if(1==result.high_status){
        				$("#old_high_score").html(result.highScore.scoreBalance);
        			}
        			if(1==result.low_status){
        				$("#old_low_score").html(result.lowScore.scoreBalance);
        			}
            		...
            		$.post(questURL,questArr,function(result){
                		...
                		$.post(questURL,questArr,function(result){
                			var changeIcon1 = '<span class="glyphicon glyphicon-arrow-up" style="color:green;"></span>';
                			var changeIcon2 = '<span class="glyphicon glyphicon-arrow-down" style="color:red;"></span>';
                			if(1==result.high_status){
                				$("#high_score").html(result.highScore.scoreBalance);
                				var change = parseInt($("#high_score").html())-parseInt($("#old_high_score").html());
                				var changeIcon = '';
                				if(change>0){
                					changeIcon = changeIcon1;
                				}
                				if(change<0){
                					changeIcon = changeIcon2;
                				}
                				$("#high_score_change").html(changeIcon+change);
                			}
                			if(1==result.low_status){
                				$("#low_score").html(result.lowScore.scoreBalance);
                				var change = parseInt($("#low_score").html())-parseInt($("#old_low_score").html());
                				var changeIcon = '';
                				if(change>0){
                					changeIcon = changeIcon1;
                				}
                				...
                				$("#low_score_change").html(changeIcon+change);
                			}
                			$('#scoreModal').modal();
                		});
            		});
        ...
        $(function(){   
        	...
        	$('.cancelOrder').click(function(){
        		...
        		var questURL = runParams.mainUrl+'confirmAgain.do';
        		var questArr = {
        			orderId:orderId,
        		...
        		$.post(questURL,questArr,function(result){
        			if(1==result){
        				alert('操作成功！');
        				location.href=runParams.mainUrl+'order_qryk.html';
        			}else{
        				alert('操作失败！');
        				return false;
        			}
        	...
        	$('.submitOrder').click(function(){
        		...
        		var token = $('input[name="token"]').val();
        		var payStatusType = $('input[name="payStatusType"]').val();
        		if(!orderId){
        			alert("未找到这个订单！");
        			return false;
        		}      
        		...
        		$.post(questURL,questArr,function(result){
        			if(1==result){
        				alert('操作成功！');
        				if('定金'==payStatusType){
        					location.href=runParams.mainUrl+'order_qrdj.html';
        				}else{
                        	location.href=runParams.mainUrl+'order_qryk.html';
        				}
        			}else{
