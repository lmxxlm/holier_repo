...
<body>
	...
	<div class="container-fluid">
		...
		<div id="content">
			...
			<ul id="productStatus" class="nav nav-tabs">
               ...
               <li class="active"><a href="javascript:void(0)" data-link="count_sendCoins.html" class="urlConvert" data-toggle="tab" onfocus="this.blur()">手动派发金币统计</a></li>
		    ...
		    <form method="post" id="searchForm" action="${mainUrl}optj_search_sendCoins.do">
			...
			<div class="searchBar row">
				...
				<div class="addPat col-md-6 col-xs-6" style="padding-right:0;">
				   <span class=" pull-right" style="margin-top:8px">
						当前页总额：<b style="color:red; font-size:18px;" id="exportCount">${jro.count}</b>
					</span>
				</div>
			</div>
			</form>
			<div class="mainBody">
				...
				<table class="table table-striped table-hover" id="tableExcel">
					...
					<tbody>
						...
						<c:forEach items="${jro.historys}" var="yh">
							<tr>
							 <td>${yh.historyId}</td>
							 <td>${yh.accountId}</td>
				...
				<div class="addPat col-md-2 col-xs-2 pull-right" >
                  <a id="test" class="btn btn-primary exportHG  pull-right"   onClick="exportTable()"   href="javascript:void(0);">导出搜索结果</a>
				</div> 
				<div align="center">
					<ul class="pager">
						<li <c:if test="${searchParam.cp == 1}"> class="disabled"</c:if>><a
							href="${mainUril}count_sendCoins.html?cp=${jro.pageView.back}&&ps=${searchParam.ps}&&startTime=${startTime}&&endTime=${endTime}">上一页</a>
						</li>
						<li     
							<c:if test="${jro.pageView.last == searchParam.cp}"> class="disabled"</c:if>><a
							href="${mainUril}count_sendCoins.html?cp=${jro.pageView.next}&&ps=${searchParam.ps}&&startTime=${startTime}&&endTime=${endTime}">下一页</a>
						</li>
					</ul>
	...
	$(function(){   
		$('#showAll').click(function(){
			window.open("${mainUril}showSendCoins.html","_blank")
		});
		function searchProduct(){
			var startTime = $('input[name="startTime"]').val();
			var endTime = $('input[name="endTime"]').val();
			location.href=runParams.mainUrl+'count_sendCoins.html?startTime='+startTime+'&endTime'+endTime;
		}
	});
	...
	function exportTable(testLink){
		...
		var questURL = runParams.mainUrl+'sendCoinsExport.do';
		var questArr = {
			startTime:startTime,
			endTime:endTime
			//token:token
		...
		$.post(questURL,questArr,function(result){
			data=result.historys;
			if(data!=null){
				var str="操作号,对象医生账号,对象医生,添补额度,添扣,操作时间,操作者\n";
				var x="";
				var dataLenth=data.length;
				for(var j=0;j<dataLenth;j++){
					var test2='"'+data[j].historyId+'","'+data[j].accountId+'","'+data[j].accountName+'","'+data[j].amount+'","'+data[j].type+'","'+data[j].time_str+'","'+data[j].writePerson+'"\n';
				   // var test2=data[j].historyId+","+data[j].accountName+","+data[j].amount+","+data[j].type+","+data[j].time_str+","+data[j].writePerson+"\n";
					x+=test2;
				}
				var exportCount=$("#exportCount").html();
				var test=str+x+"\n当前页总额:"+exportCount;
				   var blob = new Blob([test], {type: "text/plain;charset=utf-8"});
                   saveAs(blob,startTime+" 至  "+endTime+"-手动派发金币统计.csv");
			}else{
				alert("没有数据");
			}	
		});
