...
public class KfTransferOrderController {
  ...
  private final V2KfHospitalModule kfHospitalModule = new V2KfHospitalModuleImpl();
  ...
  public ModelAndView newOrderManager(@ModelAttribute SearchParams param, HttpServletRequest request) {
    ...
    List<Map<String, Object>> orderlist = null ;// kfModule.getOrderList(query);
    if (orderlist == null || orderlist.isEmpty()) view.addObject("noOrder", true);
      ...
      for (Map<String, Object> map : orderlist) {
        ...
        subdata.put("patient_phone", map.get("patient_phone"));
        subdata.put("doctor_id", map.get("doctor_id"));
  ...
  public boolean kf_order_open(String orderid, HttpServletRequest request, HttpServletResponse response) {
    ...
    if (medexToken.matchToken(request) == MedexToken.code_mismatch) {
      ...
      if (v) {
        // 给医院运营人员发短信
        Map<String, Object> order_detail = kfModule.getOrderDetail(orderid);
        Map<String, Object> hospital_detail = kfHospitalModule.findByHospitalId(order_detail.get("hospital_id")
            .toString());
        Object o_op_contact_tel = hospital_detail.get("op_contact_tel");
