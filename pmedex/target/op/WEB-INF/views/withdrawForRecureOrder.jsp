...
<body>
					...
					<tbody>
						...
						<c:forEach items="${data}" var="record">
							<tr>
								...
								<td>
                                   <a href="javascript:void(0)" class="btn btn-primary checkTx" style="font-size:12px;" data-id="${record.recordid}">已提现</a>&nbsp;<a href="javascript:void(0)" data-id="${record.recordid}" data-orderid="${record.orderid}" data-payAccountId="${record.pay_accountid}" data-payAccountName="${record.pay_account_name}" data-amount="${record.amount}" class="rejectTx" style="display:block;padding-top:10px;">→&nbsp;驳回</a>
                                </td>
	...
	$(function(){
		...
		$('.checkTx').click(function(){
			...
			var questURL = runParams.mainUrl+'kf_transfer_bill_withdraw_ok.do';
			var questArr = {
				recordid:recordid,
				token:token
			};
		...
		$('.rejectTx').click(function(){
	        ...
	        $('#rejectModal input[name="recordId"]').val(recordid);
	        $('#rejectModal .orderId').html(orderid);
		...
		$('#doSubmit').click(function(){
			var recordid=$('#rejectModal input[name="recordId"]').val();
			...
			var questURL = runParams.mainUrl+'reject_kf_transfer_withdraw.do';
			var questArr = {
				recordid:recordid,
		...
		$('.mulity_submit a').click(function(){
			...
			var questURL = runParams.mainUrl+'mulity_kf_transfer_bill_withdraw_ok.do';
