				...
				<div align="center">
					<ul class="pager">
						<li <c:if test="${searchParam.cp == 1}"> class="disabled"</c:if>>
					    	<a href="${mainUril}kfHospital.html?cp=${searchParam.cp-1}&&ps=${searchParam.ps}"  >上一页</a>
						</li>
						<li     
							<c:if test="${oro.pageView.last == searchParam.cp}"> class="disabled"</c:if>>
							<a href="${mainUril}kfHospital.html?cp=${searchParam.cp+1}&&ps=${searchParam.ps}" >下一页</a> 
						</li>
					</ul>
		...
		*/
    	...
    	function doAdd(actionType){
    		...
    		var questArr = {
    			...
    			op_contact_tel:operate,
    			fe_contact_tel:financial,
    			info_contact_tel:maintain_info
    		};
    	...
    	function commen(id,x){
    		...
    		var token=$('input[name="token"]').val();
    		//根据id获取这条数据
    		var questURL = runParams.mainUrl+'getKfHosInfoById.do';
    		var questArr = {
    			hospital_id:id,
    			token:token
    		};
    		...
    		$.get(questURL,questArr,function(result){
    			...
    			if(result.status){
    				...
    				$('#adminModal input[name="hosInfo_id"]').val(result.data.record_id);
    	     	    $('#adminModal input[name="hos_name"]').val(result.data.name);
    	     	    $('#adminModal input[name="detail_address"]').val(result.data.address);
    	     	    $('#adminModal input[name="pss"][value="'+result.data.pss+'"]').attr('checked',true);
    	     		$('#adminModal input[name="hos_specialist"]').val(result.data.departments);
    	     		$('#adminModal textarea[name="hos_info"]').val(result.data.intro);
    	      	    $('#adminModal input[name="bed_num"]').val(result.data.bednumber);
    	      	    $('#adminModal input[name="weight"]').val(result.data.weight);
    	      	    $('#adminModal input[name="reward"]').val(result.data.reward);
    	      	    
    	      	//三个维护人
    	      	    $('#adminModal input[name="financial"]').val(result.data.fe_contact_tel);
    	      	    $('#adminModal input[name="maintain_info"]').val(result.data.info_contact_tel);
    	      	    $('#adminModal input[name="operate"]').val(result.data.op_contact_tel);
