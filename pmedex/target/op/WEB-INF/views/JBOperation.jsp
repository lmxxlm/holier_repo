...
<body>
	...
	<div class="container-fluid">
		...
		<div id="content">
			<%@ include file="./commons/topMenu.jsp"%>
                <ol class="breadcrumb webmap">
                  <li><a href="#">Home</a></li>
                  <li class="active">金币添补历史</li>
                </ol>
                <ul id="productStatus" class="nav nav-tabs">
                   <li class="active"><a href="javascript:void(0)" onfocus="this.blur()">金币添补</a></li>
				   <li><a href="beansOperation.html" onfocus="this.blur()">银币添补</a></li>
			    </ul>
                <div class="searchBar row">
                    <div class="smartSearch col-md-4 col-xs-4">
                        <form action="JBOperationSearchByDoctorId.do" method="post" class="searchForm">
                        <input type="hidden" name="cp" value="${searchParam.cp}">
                        <input type="hidden" name="ps" value="${searchParam.ps}">
                        <div class="input-group">
                           <input type="hidden" name="accountId" value="${accountId}">
                           <input type="text" class="form-control" name="searchDoc" placeholder="搜索医生" autocomplete="off" value="${accountName}">
                    ...
                    <div class="addPat col-md-8 col-xs-8">
                        <button type="button" class="btn btn-primary pull-right" onFocus="this.blur()"><span class="glyphicon glyphicon-plus-sign"></span>金币添补</button>
                    </div>
                </div>
                <div class="mainBody">
                  <!--如果找到用户 srart-->
                  ...
                  <div align="center">
					  ...
					  <!--    <a href="<c:choose><c:when test="${searchParam.cp == 1}">javascript:void(0)</c:when><c:otherwise>${mainUril}JBOperation.html?cp=${yro.pageView.back}&&ps=${searchParam.ps}</c:otherwise></c:choose>">上一页</a>-->
					    <a href="javascript:void(0)"  data-cp="${yro.pageView.back}"  data-ps="${searchParam.ps}">上一页</a> 	
						</li>
						<li <c:if test="${searchParam.cp == yro.pageView.last}"> class="disabled"</c:if>>
					        <a href="javascript:void(0)"  data-cp="${yro.pageView.next}"  data-ps="${searchParam.ps}">下一页 </a> 
					   <!--   <a href="<c:choose><c:when test="${yro.pageView.last == searchParam.cp}">javascript:void(0)</c:when><c:otherwise>${mainUril}JBOperation.html?cp=${yro.pageView.next}&&ps=${searchParam.ps}</c:otherwise></c:choose>">下一页</a> 	-->		
						</li>
					</ul>
           ...
           <div class="modal-dialog">
              <div class="modal-content">
                 <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="adminModalLabel">金币添补</h4>
                 </div>
                 <div class="modal-body">
                    <div class="form-group smartSearch">
                        <div>
                        <label for="accountId">操作医生对象</label>
         ...
         <div class="modal fade" id="detailModal" data-backdrop="false" tabindex="-1" role="dialog" aria-labelledby="detailModalLabel" aria-hidden="true">
           <div class="modal-dialog">
              <div class="modal-content">
                 <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title" id="detailModalLabel">金币添补详情</h4>
                 </div>
                 <div class="modal-body">
                    ...
                    <tr>
                      ...
                      <td colspan="5"><span class="d-reason">沙sfdsff啊，第三方啊，第三方爱的色放， t5gtgtr色thty色放阿三地方爱的色放阿三地方，阿迪斯发的fads发。</span></td>
                    </tr>
                  </table>
                 </div>
                 <div class="modal-footer">
        ...
        <script src="assets/script/productAdmin/JBOperation.js"></script>
        <script>
    	   ...
    	   $('.pagination a').click(function(){
    			...
    			var cpForm=$('.searchForm input[name="cp"]').val();
    			//当前页
