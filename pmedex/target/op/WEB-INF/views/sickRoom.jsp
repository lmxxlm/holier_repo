				...
				<div align="center">
					<ul class="pager">
						<li <c:if test="${searchParam.cp == 1}"> class="disabled"</c:if>>
					    	<a href="${mainUril}sickRoom.html?cp=${searchParam.cp-1}&&ps=${searchParam.ps}"  >上一页</a>
						</li>
						<li     
							<c:if test="${oro.pageView.last == searchParam.cp}"> class="disabled"</c:if>>
							<a href="${mainUril}sickRoom.html?cp=${searchParam.cp+1}&&ps=${searchParam.ps}" >下一页</a> 
						</li>
					</ul>
			...
			*/
	    	...
	    	function commen(id,x){
	    		...
	    		$.get(questURL,questArr,function(result){
	    			//eval
	    			if(result.status){
	    				$('#adminModal input[name="sickRoom_id"]').val(result.data.record_id);
	    				$('#adminModal input[name="sickroom_name"]').val(result.data.name);
	    	     	    $('#adminModal input[name="belongs_hos"]').val(result.data.hospital_name);
	    	     	    $('#adminModal input[name="price"]').val(result.data.price);
	    	     	    $('#adminModal input[name="building_area"]').val(result.data.space);
	    	     	    $('#adminModal input[name="bed_num"]').val(result.data.bednumber);
	    	     	    $('#adminModal input[name="floor"]').val(result.data.floor);
	    	     	    $('#adminModal textarea[name="device_tab"]').val(result.data.tags);
	    	     	    $('#adminModal input[name="picture"]').val(result.data.pictures);
	    	     		$('input[name="hosId"]').val(result.data.hospital_id);
