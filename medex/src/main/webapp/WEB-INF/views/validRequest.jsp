   ...
   医生姓名
     ...
     医生姓名
				...
				<form class="navbar-form navbar-right" role="add">
					<div class="btn-group">
						<a href="javascript:void(0)" class="btn btn-primary active">认证申请列表</a>
						<a href="validHistory.html" class="btn btn-primary">认证操作历史</a>
					</div>
				</form>	
			</div>
			<div class="mainBody">
				...
				<tbody>
					<c:forEach items="${dro.records}" var="doctor" varStatus="status">
						<tr>
						    <td>${doctor.request_time_str}</td>
							<td>${doctor.doctor.name}</td>
							<td><c:choose>
									<c:when test="${empty doctor.doctor.gender}">
  暂无
   </c:when>
									<c:otherwise>
   ${doctor.doctor.gender}
							...
							<td>${doctor.doctor.referHospital}</td>
							<td>${doctor.doctor.referDepartments}</td>
							<td>${doctor.doctor.doctorTitle}</td>
							<td class="td2"><a
								href="${mainUrl}doctor/doc_info.html?doctorId=${doctor.doctor.accountId}">进入</a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
				...
				<li <c:if test="${searchParam.cp == 1}"> class="disabled"</c:if>><a
					href="${mainUril}chooseAccountDoc.html?cp=${dro.pageView.back}&&ps=${searchParam.ps}">上一页</a>
				</li>
				<li
					<c:if test="${dro.pageView.last == searchParam.cp}"> class="disabled"</c:if>><a
					href="${mainUril}chooseAccountDoc.html?cp=${dro.pageView.next}&&ps=${searchParam.ps}">下一页</a>
				</li>
			</ul>
				<!-- 分页结束 -->
				<!--如果找到用户 end-->
