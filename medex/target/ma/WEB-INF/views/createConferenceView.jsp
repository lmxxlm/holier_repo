...
.table td:first-child{ width:20%; text-align:right; padding-right:12px; background:#F5F5F5; vertical-align:middle;}
.dateInput{ cursor:pointer !important;}
textarea{ padding:5px !important; line-height:25px;}
</style>
</head>
<body>
	<!--[if lt IE 7]>
...
<input type="hidden" name="currentUserType" value="${currentUserType}">
	<div class="container-fluid">
		...
		<div id="content">
			...
			<ul id="productStatus" class="nav nav-tabs">
                  <li class="active"><a href="javascript:void(0)" onfocus="this.blur()">创建新诊室</a></li>
			   <li><a href="bindMeetingView.html" onfocus="this.blur()">绑定诊室</a></li>
		    </ul>
			<div class="mainBody">
				<table class="table table-hover">
				<thead>
					<tr>
						<th colspan="2">诊室预约</th>
					</tr>
				</thead>
				<tbody>
				    <tr>
                        <td>*&nbsp;服务场景：</td>
                        <td><select name="type" class="form-control"><option value="doctor">名医电视诊室</option><option value="community">社区医疗服务诊室</option><option value="ma" selected>健康管家咨询</option></select></td>
                    </tr>   
                    <tr>
                        <td>*&nbsp;固定诊室标题：</td>
                        <td><input name="meetingTopic" type="text" class="form-control" placeholder="请输入 固定诊室标题"/></td>
                    </tr>
                    <tr>
                        <td>*&nbsp;固定诊室密码：</td>
                        <td><input name="meetingPwd" type="text" class="form-control" placeholder="密码只能是数字" 
								onkeyup="value=value.replace(/[^\d]/g,'')"
								onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))"/></td>
                    </tr>
                    <tr>
                        <td>*&nbsp;诊室持续时间：</td>
                        <td><input name="meetingDuration" type="text" class="form-control" placeholder="几分钟" style="width:100px; float:left;" onkeyup="value=value.replace(/[^\d]/g,'')"
								onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))"/><span style="float:left; margin-left:10px; margin-top:8px;">分钟</span></td>
                    </tr>
                    <tr>
                        <td>*&nbsp;预约开始时间：</td>
                        <td><input name="meetingStartTime" type="text" class="form-control dateInput" placeholder="请选择时间" readonly/></td>
                    </tr>
                    <tr>
                        <td>*&nbsp;主持人密码：</td>
                        <td><input name="hostPwd" type="text" class="form-control" placeholder="只能是数字组合" onkeyup="value=value.replace(/[^\d]/g,'')"
								onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))"/></td>
                    </tr>
                    <tr class="yhz_doctor">
							<td>*&nbsp;与会者--医生：</td>
							<td>
								<div class="smartSearch">
                                    ...
                                    <input type="text" name="doctor_name" class="form-control" placeholder="请输入医生姓名" autocomplete="off" value="" />
						...
						<tr class="yhz_patient">
							...
							<td>
								<div class="smartSearch">
                                    ...
                                    <input type="text" name="patient_name" class="form-control" placeholder="请输入患者姓名" autocomplete="off" value="" />
						...
						<tr class="yhz_ma">
							...
							<td>
								<div class="smartSearch">
                                    ...
                                    <input type="text" name="ma_name" class="form-control" placeholder="请输入主持人（医助）姓名" autocomplete="off" value="" />
                    ...
                    <tr>
                        <td>用户名：</td>
                        <td><input name="userName" type="text" class="form-control" placeholder="请输入用户名，保持唯一性" value="好乐医"/></td>
                    </tr>
					<tr>
                        <td>显示名：</td>
                        <td><input name="userDisplayName" type="text" class="form-control" placeholder="请输入显示名" value="好乐医"/></td>
                    </tr>
                    <tr>
                        <td>诊室简介：</td>
                        <td><textarea name="meetingContent"  class="form-control" placeholder="请输入诊室简介" style="padding:12px !important;">好乐医视频服务</textarea></td>
                    </tr>
                    <tr>
                        <td>持续时间（展示）：</td>
                        <td><input name="showTime" type="text" class="form-control" placeholder="几分钟" value="10" style="width:100px; float:left;" onkeyup="value=value.replace(/[^\d]/g,'')"
								onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))"/><span style="float:left; margin-left:10px; margin-top:8px;">分钟</span></td>
                    </tr>
                    <tr>
                        <td>最大支持人数：</td>
                        <td><input name="maxAttendNum" type="text" class="form-control" placeholder="请输入最大支持人数" value="3"/></td>
                    </tr>
                    <tr>
                        <td><input type="hidden" value="${token}" name="token" /></td>
                        <td><button type="button" class="btn btn-primary" id="save" name="submit">创建诊室</button></td>
                    </tr>  
				</tbody>
			</table>
			</div>
	...
	$(function(){
		...
		function subForm(){
			...
			var optionJson = [{"meetingDuration":meetingDuration,"meetingStartTime":meetingStartTime,"hostPwd":hostPwd,"maxAttendNum":maxAttendNum,"meetingContent":meetingContent,"meetingType":2}];
			var questURL = runParams.mainUrl+'createConferenceAction.do';
			var questArr = {
					userName:userName,
