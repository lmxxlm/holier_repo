...
<body>
				...
				<div align="center">
					<ul class="pager">
						<li <c:if test="${searchParam.cp == 1}"> class="disabled"</c:if>><a
							href="javascript:void(0)" data-link="${mainUril}caindex.html?cp=${oro.pageView.back}&&ps=${searchParam.ps}" data-status="${status}">上一页</a>
						</li>
						<li     
							<c:if test="${oro.pageView.last == searchParam.cp}"> class="disabled"</c:if>><a
							href="javascript:void(0)" data-link="${mainUril}caindex.html?cp=${oro.pageView.next}&&ps=${searchParam.ps}" data-status="${status}">下一页</a>
						</li>
					</ul>
	...
	$(function(){
		...
		$('.urlConvert').click(function(){
			...
			location.href=runParams.mainUrl+"caindex.html?status="+status;
		});
