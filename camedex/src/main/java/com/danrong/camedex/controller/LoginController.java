...
import com.danrong.camedex.util.CookieParser;
...
public class LoginController {
  ...
  private PowerModule powerModule = new PowerModuleImpl();
  ...
  public int doLogoutAction(HttpServletRequest request) {
    ...
    sessionParser.remove(request, SessionField.power);
  ...
  public int check(@ModelAttribute Account account, HttpServletRequest request, HttpServletResponse response) {
    ...
    if (code == DbOption.DBOPTION_SUCCESS) {
      ...
      // session 设置 权限
      AccountPower ap = powerModule.findPowerByAccount(account.getAccountId(), MedexConfigure.destCaType);
      if (ap != null) sessionParser.setSession(request, SessionField.power, ap.getPower_id() + "");
