						...
						<td bgcolor="#f3ffe3"><table width="99%" border="0"
								...
								<tr class="listTr">
									<td class="td2">
										<h2>基本信息</h2>
										<p>
											医生id：<span>${account.accountId}</span>
										</p>
										<p>
											类别：<span>${doctor.type}</span>
										</p>
										<p>
											姓名：<span>${account.name}</span>
										</p>
										<p>
											性别：<span>${doctor.gender}</span>
										</p>
										<p>
											所属医院：<span><c:forEach
													items="${doctor.referHospital_array}" var="s"
													varStatus="status">
									${s}
									</c:forEach> </span>
										</p>
										<p>
											所属科室：<span><c:forEach
													items="${doctor.referDepartments_array}" var="s"
													varStatus="status">
									${s}
									</c:forEach> </span>
										</p>
										<p>
											毕业学校：<span>${doctor.school}</span>
										</p>
										<p>
											最高学历：<span>${doctor.education}</span>
										</p>
										<p>
											职务：<span>${doctor.doctorPosition}</span>
										</p>
										<p>
											职称：<span>${doctor.doctorTitle}</span>
										</p>
										<p>
											职位：<span><c:forEach
													items="${doctor.academicPositions_array}" var="s"
													varStatus="status">
									${s}
									</c:forEach>
											</span>
										...
										<p>
											专长：<span><c:forEach
													items="${doctor.specialties_array}" var="s"
													varStatus="status">
								    ${s}
									</c:forEach>
											</span>
										...
										<p>
											电话：<span>${doctor.telePhone}</span>
										</p>
										<p>
											科室电话：<span>${doctor.departPhone}</span>
										</p>
										<p>
											电子邮件：<span>${doctor.email}</span>
										</p>
										<p>
											微信号：<span>${doctor.weixinNum}</span>
										</p>
										<p>
											QQ号：<span>${doctor.qqNum}</span>
										</p>
										<p>
											身份证号：<span>${doctor.identityID}</span>
										</p></td>
									<td class="td2">
										<h2>后台信息</h2>
										<p>
